...
// Numbered buttons on desktop with ellipsis pattern, compact prev/next +
// "X of Y" on mobile (per spec callout 5).
//
// URL state is preserved — page is a query param ("?page=2"); other
// filters survive page changes because we mutate only the page param.
export function TreatmentsPagination({
  ...
  function goTo(nextPage: number) {
    ...
    startTransition(() => {
      router.replace(`/portal/treatments${qs ? `?${qs}` : ""}`);
    });
  }

  // Page numbers with ellipsis: 1 ... (page-1) page (page+1) ... totalPages
